...
<div class="flex justify-between items-center sm:w-4/5 sm:mx-auto">
    <h1 class="text-2xl font-semibold tracking-widest">TAMBAH MENU</h1>
...
<div class="w-full min-w-md mt-5">
  <form action="{{route('storeData')}}" class="bg-white shadow-lg rounded px-8 pt-6 pb-8 mb-4 lg:w-1/2 lg:m-auto" method="POST">
    @csrf
    <div class="mb-4">
      <label class="block text-gray-700 text-lg font-bold mb-2" for="name">Product Name</label>
      <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline {{ $errors->first('name') ? 'border-red-500' : ''}}" type="text" name="name" placeholder="Product Name" value="{{ old('name') }}" autocomplete="off">
    ...
    <div class="mb-4">
      ...
      <input class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline {{ $errors->first('price') ? 'border-red-500' : ''}}" type="text" name="price" placeholder="Product Price" value="{{ old('price') }}" autocomplete="off">
